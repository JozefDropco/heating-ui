...
export default Vue.extend({
  ...
  methods: {
    loadCurrentState() {
      ...
      axios.get(cfg.BASE_URL + "watering/port/outputs")
          .then(response => {
      ...
      axios.get(cfg.BASE_URL + "watering/port/inputs")
          .then(response => {
            for (let i = 0; i < response.data.length; i++) {
              var found: boolean = false;
    ...
    handleStateChange() {
      Loading.show();
      axios.post(cfg.BASE_URL + "watering/serviceMode?state=" + this.state)
          .then(response => {
            this.state = response.data.state;
            Loading.hide();
          })
    ...
    toggleOutputState(output: any) {
      Loading.show();
      axios.post(cfg.BASE_URL + "watering/port/output/" + output.refcd, output.value, {
        headers: {
